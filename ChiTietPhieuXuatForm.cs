...
namespace QuanLyKho
{
    public partial class ChiTietPhieuXuatForm : Form
    {
        ...
        public ChiTietPhieuXuatForm(string maPhieu)
        {
        ...
        private void InitializeComponent()
        {
            ...
            this.Controls.Add(dgvChiTietPhieuXuat);
        ...
        private void LoadData()
        {
            try
            {
                DatabaseConnection.ExecuteInTransaction((connection, transaction) =>
                {
                    ...
                    // Load tổng tiền
                    query = "SELECT TongTien FROM [dbo].[Phieu_Xuat] WHERE MaPhieu = @MaPhieu";
                    using (var command = new SqlCommand(query, connection))
                    {
                        command.Transaction = transaction;
                        command.Parameters.AddWithValue("@MaPhieu", maPhieu);
        ...
        private void DgvChiTietPhieuXuat_SelectionChanged(object sender, EventArgs e)
        {
            ...
            {
                var row = dgvChiTietPhieuXuat.CurrentRow;
        ...
        private void BtnXoa_Click(object sender, EventArgs e)
        {
                ...
                MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    var maHang = dgvChiTietPhieuXuat.CurrentRow.Cells["MaHang"].Value.ToString();
                    DatabaseConnection.ExecuteInTransaction((connection, transaction) =>
                    {
                        // Xóa chi tiết
                        string deleteQuery = @"
                ...
                DatabaseConnection.ExecuteInTransaction((connection, transaction) =>
                {
                    ...
                    string checkStockQuery = @"
                        SELECT 
                            ...
                            ISNULL((
                                SELECT SUM(SoLuong)
                                FROM [dbo].[Chi_Tiet_Phieu_Xuat]
                                WHERE MaHang = @MaHang
                                AND MaPhieu != @MaPhieu
                            ), 0) as TonKho";

                    using (var cmd = new SqlCommand(checkStockQuery, connection))
                    {
                        cmd.Transaction = transaction;
                        cmd.Parameters.AddWithValue("@MaHang", cboHangHoa.SelectedValue);
                        cmd.Parameters.AddWithValue("@MaPhieu", maPhieu);
                        var tonKho = Convert.ToInt32(cmd.ExecuteScalar());

                        if (tonKho < numSoLuong.Value)
                        {
                            throw new Exception($"Số lượng tồn kho không đủ! Hiện chỉ còn {tonKho} sản phẩm.");
                        }
                    }
                    ...
                    using (var cmd = new SqlCommand(checkQuery, connection))
                    {
                        ...
                        {
                            query = @"
                                UPDATE [dbo].[Chi_Tiet_Phieu_Xuat]
                                SET SoLuong = @SoLuong,
                                    DonGia = @DonGia,
                                    ThanhTien = @ThanhTien
                                WHERE MaPhieu = @MaPhieu AND MaHang = @MaHang";
                        ...
                        {
                            query = @"
                                INSERT INTO [dbo].[Chi_Tiet_Phieu_Xuat] (MaPhieu, MaHang, SoLuong, DonGia, ThanhTien)
